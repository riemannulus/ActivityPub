...
[//]: # "' `OrderedCollection` 타입 이여야 합니다' 로 변경하여야 올바를 것 같지만 일단 냅두겠습니다.(TODO)"
...
주어진 유저의 액티비티를 검색 할수 있는 요청자의 권한에 따라 유저의 아웃박스 스트림에는 해당 유저가 게시한 액티비티가 들어가 있습니다. (이는 아웃박스의 내용물은 읽는 사람의 권한에 따라 필터링 됩니다). 만약 유저가 [인증](https://www.w3.org/TR/activitypub/#authorization) 없이 요청을 제출할 경우에 서버는 모든 [공개](https://www.w3.org/TR/activitypub/#public-addressing) 게시물들로 응답하여야 합니다. 이 응답에 해당하는 게시물의 범위는 서버의 구현 및 배포하는 사람의 재량에 달려 있지만, 유저가 게시한 것들과 연관된 모든 객체들 일수도 있습니다.
...
[//]: # "다수의 의역이 있고 {objects->객체들? ~것들?} 차후에 재검토 해보겠습니다.(TODO)"
...
인박스 스트림에는 액터가 받은 모든 액티비티들이 포함됩니다. 서버는 요청자의 권한에 따라 컨텐츠를 필터링 하는것을 *권장합니다*. 일반적으로 인박스의 소유자는 인박스 안의 모든 내용에 대하여 접근 할 수 있습니다. 접근 제어 여부에 따라 다르지만 인박스에 접근이 가능할 경우, 일부 컨텐츠는 공개(public) 일 수도 있고, 소유자가 아닌 사용자들에게 인증을 요구 할 수도 있습니다.
...
서버는 *반드시* 인박스에 의해 리턴된 액티비티들의 중복 제거를 수행하여야 합니다. 액티비티의 중복은 액티비티가 액터의 팔로어들과 수신자 액터를 팔로잉하는 특정한 액터 모두에게 전파되어 있고, 서버가 수신자들의 목록을 중복 제거하지 못하였을 경우에 생깁니다. 이러한 중복 제거는 *반드시* 액티비티들의 `id`들을 비교하여 이미 본 액티비티들을 삭제하는 방식으로 수행하여야 합니다.
...
[//]: # "'addressed'를 '전파하다~'로 번역해두었습니다."

연합 서버에서 액터의 인박스들은 [전송(Delivery)](https://www.w3.org/TR/activitypub/#delivery)에 기재되어 있는 동작방식을 따라 HTTP POST 요청을 수락합니다. 비-연합 서버는 POST 요청을 수신하면 405 Method Not Allowed를 리턴하여야 합니다.
...
[//]: # "' `OrderedCollection` 타입 이거나 `Collection` 타입 이여야 합니다' 로 변경하여야 올바를 것 같지만 일단 냅두겠습니다.(TODO)"
...
이 특별한 URI로 전송 된 액티비티들은 모든 사용자가 인증없이 접근이 가능하여야 합니다. "공개(public)" 특수 컬렉션은 실제 액티비티들을 수용할 수 없기 때문에 구현체들은 이 컬렉션에게 전달을 *해서는 아니 됩니다*. 하지만 액터들에게는 (팔로워-전용 게시물들을 포함한) 공개 게시물들을 효율적으로 공유할 수 있는 [`sharedInbox`](https://www.w3.org/TR/activitypub/#sharedInbox) 엔드포인트가 *있을 수도 있습니다*; [7.1.3 공유 인박스 전송](https://www.w3.org/TR/activitypub/#shared-inbox-delivery) 참고바람.
...
[//]: # "2번째 문장의 번역이 매끄럽지 않은 것 같습니다.(TODO)"
...
모든 객체들은 `likes` 컬렉션을 가지고 *있을 수* 있습니다. 이 컬렉션은 이 객체를 `object` 속성으로 포함하는 모든 `Like` 액티비티들을 [부작용](https://www.w3.org/TR/activitypub/#like-activity-inbox)을 통하여 추가한 목록입니다. `likes` 컬렉션은 *반드시* [`OrderedCollection`](https://www.w3.org/TR/activitystreams-vocabulary/#dfn-orderedcollection) 이거나 [`Collection`](https://www.w3.org/TR/activitystreams-vocabulary/#dfn-collection)이여야 하고, 인증된 유저의 권한에 따라 또는 인증이 제공되지 않은 경우 적절하게 필터링을 *할 수도* 있습니다. 
