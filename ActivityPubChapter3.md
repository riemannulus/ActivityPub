...
객체는 [[액티비티스트림](https://www.w3.org/TR/activitypub/#bib-ActivityStreams)]과 액티비티펍이 기반하는 핵심 개념입니다. 객체는 종종 액티비티들로 감싸져 있고 객체의 서브클래스인 컬렉션들의 흐름에 포함되어 있습니다. [[액티비티-단어](https://www.w3.org/TR/activitypub/#bib-Activity-Vocabulary)] 문서를 참고하시고, 특히 [핵심 클래스](https://www.w3.org/TR/activitystreams-vocabulary/#types) 부분을 참고하십시오; 액티비티펍은 이 단어의 매핑을 밀접하게 따릅니다.
...
[//]: # "Streams->흐름으로 번역하였습니다."
[//]: # "마지막 문장이 매끄럽지 않은 것 같습니다"

액티비티펍은 액티비티스트림이 제공한 용어 외에도 추가적으로 용어를 정의합니다. 이러한 용어들은 `https://www.w3.org/ns/activitystreams` 의 액티비티펍 [JSON-LD 컨텍스트](http://www.w3.org/TR/json-ld/#the-context)에서 제공되고 있습니다. 구현자들은 액티비티펍 컨텍스트를 객체의 정의에 포함시키는 것을 *권장합니다*. 구현자들은 추가적인 컨텍스트를 적절하게 포함 *시킬 수도* 있습니다.

~~ActivityPub defines some terms in addition to those provided by ActivityStreams. These terms are provided in the ActivityPub [JSON-LD 컨텍스트](http://www.w3.org/TR/json-ld/#the-context) at `https://www.w3.org/ns/activitystreams`. Implementers *SHOULD* include the ActivityPub context in their object definitions. Implementers *MAY* include additional context as appropriate.~~
...
서버는 컨텐츠 스푸핑 공격을 방지하기 위하여 수신한 콘텐츠의 유효성을 검증하기를 *권장합니다.* (서버는 적어도 오브젝트가 원 발신지로부터 수신되었는지 검증하는 등의 단순한 절차를 거쳐야 하지만, 가능하다면 서명 등을 검증하는 등의 메커니즘이 더 좋습니다). 검증 메커니즘에 대해서는 이 문서에서는 특별히 규정하고 있지 않습니다만, [보안 고려사항](https://www.w3.org/TR/activitypub/#security-considerations) 를 참고하여 제안과 선례 등을 확인해 주시길 바랍니다.
...
[//]: # " ㄴ 위 사항에 대해서는 목차 목록 일치화를 위하여 '보안 고려사항'으로 변경하였습니다. "
[//]: # "content spoofing attacks -> '콘텐츠 변조 공격' 을 '컨텐츠 스푸핑 공격'으로 변경하였습니다."

예를 들어, example.com 이 액티비티를 전달받았을 때
...
전달받은 측은 `id` 를 역 참조하여 이것이 유효한 객체로 존재하는지 여부와 객체를 잘못 반영하고 있지 않은지를 검증하여야 합니다. (예제의 경우, Mallory는 Alice 에 의해 포스트된 것으로 보이는 오브젝트를 변조하였을 수 있습니다.)
...
[액티비티스트림]의 모든 객체는 고유의 전역구분자를 가져야 합니다. 액티비티펍은 이 제한의 연장선에 있습니다; 액티비티펍 프로토콜에 의해 배포되는 모든 객체들은, 의도적으로 일시적으로 만든 경우(접근되도록 의도되지 않은 단기 활동들로 챗 메세지나 게임 알림 등이 이에 해당합니다)에 해당하지 않는 한, 고유의 전역 구분자를 *반드시* 가져야 합니다. 이러한 구분자들은 아래의 그룹 중 하나에 속합니다:
...
1. 발신원인 서버에 권한이 귀속되는, HTTPS URI 등 공개적으로 역 참조가 가능한 URI. (외부에 닿아있는 컨텐츠는 HTTPS URI를 사용하기를 *권장합니다*.)
...
[//]: # "\* 원문은 authority"
[//]: # " ㄴ 위 사항에 대해서는 번역 일관성을 위하여 '권한'으로 변경하였습니다. "
[//]: # "'발신원인 서버' 라고 적은 것은 오역의 가능성이 있다고 생각됩니다. 차후에 검토해보겠습니다.(TODO) "

서버-서버 간 통신에서 액티비티가 의도적으로 일시적인 경우가 아닌 한 구분자는 *반드시* 제공되어야 합니다. 그러나, 클라이언트-서버 간 통신에서는 특정된 `id`가 없이 아웃박스에 게시된 객체를 수신한 서버는 객체 ID를 액터의 namespace에 할당하고 게시된 객체에 첨부하기를 *권장합니다*.
...
모든 객체들은 아래의 속성들을 가집니다:

~~All objects have the following properties:~~

**id**

객체의 고유한 전역 구분자 (오브젝트가 일시적인 경우를 제외하며, 이 경우 id는 비워둘 *수도* 있습니다)
...
[//]: # "Chapter2의 RFC2119와 충돌되는 몇몇 문장이 있는것 같습니다. 차후에 수정하겠습니다.(TODO)"

서버는 위의 요구 사항을 준수하지 않는 요청에 대하여 다른 동작을 구현 *할 수도* 있습니다. (예를 들어 서버는 추가 레거시 프로토콜을 구현하거나 HTML의 HTML 및 액티비티스트림 표현에 동일한 URI를 사용할 수 있습니다).
...
서버는 B.1 [인증 및 권한](https://www.w3.org/TR/activitypub/#authorization)에 지정된 권한을 요구 *할 수도* 있으며, 추가로 자체 권한 부여 규칙을 구현 할 수도 있습니다. 서버는 적절한 HTTP 오류 코드 또는 객체가 비공개로 간주되는 '403 Forbidden error' 에러코드와 함꼐 권한 부여 검사를 통과하지 못한 요청에 대하여 실패하는 것을 *권장합니다*. 비공개 대상의 존재를 공개하지 않으려는 발신원 서버는 상태 코드 '404 Not Found'로 응답 *할 수도* 있습니다.
...
액티비티펍은 [액티비티-단어](https://www.w3.org/TR/activitypub/#bib-Activity-Vocabulary)에서 정의한 모든 속성 외에도 `source` 속성을 제공하여 `Object`를 확장합니다. `source`속성은 `content` 마크업의 출저를 발송지(provenance)의 형태로 전달하거나 클라이언트들의 항후 편집을 지원하기 위하여 존재합니다. 일반적으로 클라이언트는 `source`에서 `content`으로만 변환을 수행합니다.
...
`source`의 값은 객체이며, 자기 자신의 `content` 및 `mediaType` 필드를 사용하여 출저 정보를 제공하는 객체입니다.
...
>
> 일반적으로 사용자가 원본 게시물을 원본과 동일한 출저 형식으로 편집하도록하는 것이 가장 좋습니다. 그러나 모든 클라이언트가 모든 출저 유형에 대해 훌륭한 인터페이스를 안정적으로 제공 할 수있는 것은 아니며 클라이언트가 `source`에서`content`로 변환하는 작업을 하는 것을 전제로 하므로, 일부 클라이언트는 다른 클라이언트가 처리하는 방법을 모르는 매체 유형으로 작업을 할 수도 있습니다. 클라이언트가 편집할 `content` 마크업을 제공하고 출저를 무시할 수는 있지만, 하지만 이는 향후 개정판에서 사용자는 원래 `source`보다 바람직한 형태를 잃게 될 수도 있다는 것을 의미합니다. 따라서 클라이언트는 원본 출저 형식을 이해하지 못하여 무시되고 있음을 경고하는 최소한의 방해 경고를 제공해야합니다.
>
> 예를 들어 Alyssa P. Hacker는 자신이 작성한 Emacs 클라이언트를 통해 [Org mode](http://orgmode.org/)를 활용하여 액티비티펍에 기반헌 자신의 블로그에 게시하는 것을 좋아합니다. 나중에 그녀는 휴대폰의 클라이언트에서 편집모드로 전환하는데, `text / x-org`가 무엇인지 또는 HTML로 렌더링하는 방법을 모릅니다. 대신 원본 `content`를 편집 할 수있는 텍스트 상자를 제공합니다. 편집 영역 위에 "이것은 원래 처리 방법을 모르는 다른 마크 업 언어로 작성되었습니다. 편집하면 원본 출저를 잃게됩니다!" 라는 유용한 경고가 표시됩니다. Alyssa는 작은 오타 수정 프로그램이 멋진 org-mode 마크업을 잃을 가치가 없다고 결정하고 집에 도착하면 업데이트하기로 결정합니다.
